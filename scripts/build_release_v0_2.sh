...
echo "[2/7] Running release smoke checks..."
(
  cd "$ROOT_DIR"
  bash scripts/release_smoke_check.sh
)

echo "[3/7] Copying application artifacts and data snapshot..."
...
cp "$ROOT_DIR/scripts/release_smoke_check.sh" "$PACKAGE_DIR/scripts/release_smoke_check.sh"
find "$PACKAGE_DIR" -type d -name "__pycache__" -prune -exec rm -rf {} +
find "$PACKAGE_DIR" -type f \( -name "*.pyc" -o -name "*.pyo" -o -name "*.log" -o -name "*.pid" \) -delete

echo "[4/7] Writing sanitized runtime files..."
...
- Included eval artifacts: \`backend/evals\`
...
chmod +x "$PACKAGE_DIR/start.sh" "$PACKAGE_DIR/stop.sh" "$PACKAGE_DIR/scripts/serve_frontend.py" "$PACKAGE_DIR/scripts/release_smoke_check.sh"

echo "[5/7] Copying release documentation..."
